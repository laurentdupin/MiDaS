...
#if defined(__linux__) && !defined(__ANDROID__)
        // MemFd fallback can overlap an already admitted DMA-BUF job.
        std::lock_guard<std::mutex> execution(*work.execution_mutex);
#endif
...
struct LinuxCaptureHooks {
    static ibr_linux_capture_capabilities capabilities(ibrh_model* model) {
        ...
        return midas_linux_capture_capabilities(model->context);
    }
    static void infer(ibrh_model* model, const inferbridge::linux_capture::LinuxDmaBufImage& source,
        ...
        if (!input_size(parameters, size, size)) throw std::invalid_argument("invalid MiDaS input size");
        std::lock_guard<std::mutex> lock(model->submit_mutex);
        midas_infer_linux_capture(model->context, source, size, output);
    }
};
}
